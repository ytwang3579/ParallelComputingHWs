...
__global__ void sobel(unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    ...
    for(int mywork = idx; mywork < height*width; mywork += gridStride) {
        y = mywork / width;
        x = mywork % width;
        //printf("%d %d\n", x, y);
            for (i = 0; i < MASK_N; ++i) {
...
int main(int argc, char** argv) {
    ...
    unsigned char* src_img_cuda = NULL;
    ...
    cudaMalloc(&src_img_cuda, img_size);
    cudaMemcpy(src_img_cuda, src_img, img_size, cudaMemcpyHostToDevice);
    ...
    unsigned char* dst_img_cuda;
    cudaMalloc(&dst_img_cuda, img_size);
    ...
    // int Block_Per_Grid = props.multiProcessorCount * 32;
    // int Thread_Per_Block = props.maxThreadsPerBlock;
